...
class ScriptsMaker extends Base
{
    ...
    public function makeCode(
        ...
        $script .= "\t\t}\n";
        if ($with_frames) {
            $script .= "\t\techo \"<script language='JavaScript'>window.location.href='?id=$page_id&lg=$lg'</script>\";\n";
        } elseif (!$with_frames) {
            $script .= "\t\t\$query=\"SELECT\";\n";
        }

        //$script.="\t\techo \"<script language='JavaScript'>window.location.href='?id=$page_id&lg=fr'</script>\";\n";
    ...
    public function makePage(
        ...
        $script .= "\t\t\t\$dbgrid = \$datacontrols->createPagerDbGrid(\$tablename, \$sql, \$id, \"\", \"&query=ACTION\$curl_pager\", \"\", true, true, \$dialog, array(0, 400), 15, \$grid_colors, \$cs);\n";
        ...
        $page_filename = "";
        if ($with_frames) {
            $script .= "<form method=\"POST\" name=\"$formname\" action=\"$page_filename?id=$page_id&lg=$lg\">\n";
        } elseif (!$with_frames) {
            $script .= "<form method=\"POST\" name=\"$formname\" action=\"$page_filename?id=$page_id&lg=$lg\">\n";
        }
    ...
    public function makeSingleScript(
        ...
        $script .= "\t\t\$dbgrid=\$datacontrols->createPagerDbGrid(\"$table\", \$sql, \$id, \"\", \"&query=ACTION\$curl_pager\", \"\", true, true, \$dialog, array(0, 400), 15, \$grid_colors, \$cs);\n";
        ...
        if ($with_frames) {
            ...
            $script .= "<form method='POST' name='$formname' action='?id=$page_id&lg=$lg'>\n";
        }
        $script .= "\t<input type='hidden' name='query' value='ACTION'>\n";
        $script .= "\t<input type='hidden' name='event' value='onRun'>\n";
        $script .= "\t<input type='hidden' name='pc' value='<?php echo \$pc?>'>\n";
    ...
    public function scriptForm(
        ...
        //else if(!$with_frames)
        //$script.="<form method='POST' name='$formname' action='?id=$page_id&lg=fr'>\n";
        $script .= "\t\t<input type='hidden' name='query' value='ACTION'>\n";
        $script .= "\t\t<input type='hidden' name='event' value='onRun'>\n";
        $script .= "\t\t<input type='hidden' name='pc' value='<?php echo \$pc?>'>\n";
        $script .= "\t\t<input type='hidden' name='sr' value='<?php echo \$sr?>'>\n";
