...
class Controls extends Base
{
    ...
    public function create_tab_control($name = "", $tab_captions = array(), $tab_ctrl = "", $hidden = "")
    {
        //Contrôle d'onglets
        ...
        if (empty($pz_current_tab)) {
            $pz_current_tab = $tab_captions[0];
        }
    ...
    public function on_change_tab($tab_ctrl_name = "", $tab_id = 0)
    {
        $js = "";
    ...
    public function create_server_file_selector($name = "", $formname = "", $basedir = "", $filter = "", $size = 0, $dir_selector = "", $onChange = "")
    {
        // global $_SESSION;
        if (substr($basedir, -2) == "/.") {
    ...
    public function create_server_directory_selector($name = "", $formname = "", $basedir = "", $onChange = "")
    {
        if (substr($basedir, -2) == "/.") {
            $basedir = str_replace("/.", "/", $basedir);
        }
    ...
    public function create_file_upload_control($name = "", $accept = "", $size = 0, $dir_selector = "")
    {
        if ($accept != "") {
    ...
    public function perform_file_upload($file_field_name = "", $file_dir = "")
    {
    ...
    public function create_dialog_window($url, $height, $width)
    {
        $dialog =
            "window.open(" .
            "'$url', " .
