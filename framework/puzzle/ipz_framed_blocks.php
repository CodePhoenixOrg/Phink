...
class FramedBlocks
{
    ...
    public function createFramedBlock($database, $block_num, $target, $id, $lg, $colors)
    {
        ...
        while ($rows = $stmt->fetch()) {
            ...
            $sub_menu .= "<tr id=\"$index$count\" onMouseOver=\"window.status='';setRowColor(this, hlBackColor, hlTextColor);\" onMouseOut=\"setBackRowColor(this);\"><td><a href=\"?id=$index&lg=$lg$request\" target=\"$target\"><span id=\"caption_$index$count$zero\" style=\"color:$fore_color\">$caption</span></a></td></tr>\n";
            $count++;
        }
    ...
    public function createFramedCollapseableBlock($block_skin_name, $block_num, $target, $colors)
    {
        ...
        while ($rows = $stmt->fetch()) {
            ...
            $link = $rows[5];
            $page = $rows[6];
            $sub_menu .= "<tr id=\"$index$count\" onMouseOver=\"setRowColor(this, hlBackColor, hlTextColor);\" onMouseOut=\"setBackRowColor(this);\"><td><a href=\"?id=$index&lg=" . $lg . "\" target=\"$target\"><span id=\"caption_$index$count$zero\" style=\"color:$fore_color\">$caption</span></a></td></tr>\n";
            $count++;
        }
    ...
    public function createFramedMembersBlock($database, $logout, $di_name, $id, $lg, $target, $colors)
    {
        ...
        if ($status == MEMBER_LOGGED_OUT) {
            $connection_link = "\t<a href=\"?di=ed_membe&lg=$lg&action=Ajouter\" target=\"$target\">Devenir membre >></a>\n";
