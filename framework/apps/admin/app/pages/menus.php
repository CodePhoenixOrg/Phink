...
if ($query === "SELECT") {
    $sql = <<<SQL
		...
		SQL;
	$dbgrid = $datacontrols->createPagerDbGrid("entrées", $sql, $id, "", "&query=ACTION$curl_pager", "", true, true, $dialog, [0, 200, 100], 15, $grid_colors, $cs);
	//$dbgrid = $datacontrols->createPagerDbGrid($tablename, $sql, $id, "", "&query=ACTION$curl_pager", "", true, true, $dialog, array(0, 400), 15, $grid_colors, $cs);
    echo "<br>" . $dbgrid;
} elseif ($query === "ACTION") {
    ?>
