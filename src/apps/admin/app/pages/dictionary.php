...
<?php   
	...
	if($query === "SELECT") {
		...
		FROM
			dictionary
		SQL;
					$dbgrid = $datacontrols->createPagerDbGrid("entrées", $sql, $id, "page.html", "&query=ACTION$curl_pager", "", true, true, $dialog, [0, 70, 120, 250], 15, $grid_colors, $cs);
					//$dbgrid = tableShadow($tablename, $dbgrid);
			echo "<br>".$dbgrid;
	} elseif($query === "ACTION") {
?>
<form method="POST" name="dictionaryForm" action="page.html?id=61&lg=fr">
