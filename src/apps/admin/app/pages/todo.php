...
<?php   
	...
	if($query === "SELECT") {
			...
			$dbgrid = $datacontrols->createPagerDbGrid("tâches", $sql, $id, "page.html", "&query=ACTION$curl_pager", "", true, true, $dialog, array(0, 450), 15, $grid_colors, $cs);
...
?>
<form method="POST" name="todoForm" action="page.html?id=25&lg=fr">
				...
				<tr>
					...
					<tr>
						...
						<td>
							<select name='mbr_id2'>
							<?php   $sql='select mbr_id, mbr_nom from members order by mbr_nom';
							$options = $datacontrols->createOptionsFromQuery($sql, 0, 1, [], $mbr_id2, false, $cs);
							echo $options["list"];?>
							</select>
						</td>
					</tr>
